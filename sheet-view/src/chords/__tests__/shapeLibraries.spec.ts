...
import { INSTRUMENT_IDS, INSTRUMENTS } from '../types'

// Duplicated from scripts/generate-chord-shapes.mjs on purpose: this is an
// independent check that a generated table sounds the chord it claims to.
...
describe('built-in shape libraries', () => {
  it('registers a table for exactly the instruments marked diagrams: builtin', () => {
    const builtinIds = INSTRUMENT_IDS.filter((id) => INSTRUMENTS[id].diagrams === 'builtin').sort()
    expect(Object.keys(BUILTIN_SHAPES).sort()).toEqual(builtinIds)
  })

  it('every chordsheetjs-sourced instrument has a six-string neck', () => {
    const wrongWidth = INSTRUMENT_IDS.filter(
      (id) => INSTRUMENTS[id].diagrams === 'chordsheetjs' && INSTRUMENTS[id].stringCount !== 6,
    )
    expect(wrongWidth).toEqual([])
  })

  describe.each(Object.entries(BUILTIN_SHAPES))('%s', (id, table) => {
    const spec = INSTRUMENTS[id as keyof typeof INSTRUMENTS]
    ...
    it(`every entry parses to a ${spec.stringCount}-string definition whose key matches its name`, () => {
      const bad = entries.filter(([key, str]) => {
        const parsed = ChordDefinition.parse(str)
        return parsed.frets.length !== spec.stringCount || parsed.name !== key
      })
      expect(bad.map(([key]) => key)).toEqual([])
    })

    it('every key is already canonical', () => {
      const nonCanonical = Object.keys(table).filter((key) => canonicalChordName(key) !== key)
      expect(nonCanonical).toEqual([])
    })

    it('has an interval table for every quality it uses', () => {
      const unknown = [...new Set(entries.map(([key]) => splitName(key).quality))].filter(
    ...
    it('every shape only sounds pitches that belong to the chord, including the root', () => {
      ...
      for (const [key, str] of entries) {
        ...
        ChordDefinition.parse(str).frets.forEach((fret, string) => {
          ...
          sounded.add(((spec.tuning[string] ?? 0) + Number(fret)) % 12)
        })
